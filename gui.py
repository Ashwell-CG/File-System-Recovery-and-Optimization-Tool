...
class FileSystemGUI:
    ...
    def _resolve_inode(self, path: str):
        """Walk the in-memory DirTree to find (inode_id, is_dir) for path, or None."""
        parts = [p for p in path.strip().lstrip("/").split("/") if p]
        fs = self.backend.fs
        dir_tree = fs.dir_tree

        inode_to_node = {}
        for i in range(dir_tree.node_count):
            node = dir_tree.nodes[i]
            if node.is_valid:
                inode_to_node[node.dir_inode] = node

        current_inode = 0
        current_is_dir = True
        for part in parts:
            node = inode_to_node.get(current_inode)
            if not node:
                return None
            found = False
            for j in range(node.entry_count):
                e = node.entries[j]
                if e.is_valid and e.name.decode("utf-8", errors="replace") == part:
                    current_inode = e.inode_id
                    current_is_dir = bool(e.is_dir)
                    found = True
                    break
            if not found:
                return None
        return (current_inode, current_is_dir)

    def delete_item(self):
        """Delete a file OR directory — auto-detected from the path."""
        path = self._prompt_path("Delete", "Enter file or directory path:", "/docs")
        if not path:
            return

        resolved = self._resolve_inode(path)
        if resolved is None:
            messagebox.showerror("Not Found", f"'{path}' does not exist in the file system.\nCheck the path and try again.")
            return

        _inode_id, is_dir = resolved
        ...
        if is_dir:
            self._run_operation(
                "Delete directory",
                lambda: self._require_success(self.backend.rmdir(path), "Delete directory"),
                f"Directory deleted: {path}"
            )
        else:
            self._run_operation(
                "Delete file",
                lambda: self._require_success(self.backend.delete_file(path), "Delete file"),
                f"File deleted: {path}"
            )
    ...
    def _build_tree_str(self) -> str:
        """Walk the in-memory DirTree and return an indented tree string."""
        fs = self.backend.fs
        dir_tree = fs.dir_tree

        # Build a map: inode_id -> DirNode
        inode_to_node: dict[int, object] = {}
        for i in range(dir_tree.node_count):
            node = dir_tree.nodes[i]
            if node.is_valid:
                inode_to_node[node.dir_inode] = node

        if 0 not in inode_to_node:
            return "(No file system mounted or disk not formatted)"

        lines: list[str] = []
        total_files = 0
        total_dirs = 0

        def walk(inode_id: int, prefix: str, is_last: bool):
            nonlocal total_files, total_dirs
            node = inode_to_node.get(inode_id)

            # Collect valid entries, dirs first then files
            entries = []
            if node:
                for j in range(node.entry_count):
                    e = node.entries[j]
                    if e.is_valid:
                        entries.append(e)
            entries.sort(key=lambda e: (0 if e.is_dir else 1, e.name.decode("utf-8", errors="replace").lower()))
            ...
            for idx, entry in enumerate(entries):
                ...
                if entry.is_dir:
                    total_dirs += 1
                    lines.append(f"{prefix}{connector}[DIR]  {name}/  (inode {entry.inode_id})")
                    walk(entry.inode_id, prefix + extension, last)
                else:
                    total_files += 1
                    # Get file size from inode table
                    size = 0
                    if 0 <= entry.inode_id < MAX_INODES:
                        inode = fs.inodes[entry.inode_id]
                        if inode.is_valid:
                            size = inode.size
                    lines.append(f"{prefix}{connector}[FILE] {name}  ({size} bytes, inode {entry.inode_id})")

        lines.append("/  (root)")
        walk(0, "", True)
        lines.append("")
        lines.append(f"  {total_dirs} director{'y' if total_dirs == 1 else 'ies'},  {total_files} file{'s' if total_files != 1 else ''}")
        return "\n".join(lines)

    def show_file_tree(self):
        """Open a popup window showing the full directory + file tree."""
        tree_str = self._build_tree_str()
        self.log("File tree requested.")
